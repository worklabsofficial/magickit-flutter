...
Only `storage/*.json` is read. JSON in subfolders is ignored. Invalid JSON, unknown field types, Dart keywords, unsupported indexes, and relations whose `target` is not another entity in `storage/` fail the command before any file is written.
...
`storage/post.json` must define an entity named `Post` before that relation is valid.
